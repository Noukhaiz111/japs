...
// Sample data for the article. You can fetch this from an API.
const articleData = {
  volume: 35,
  issue: 4,
  year: 2025,
  ...
  impactFactor: '0.60',
  jcr: '2024',
  title: 'DETECTING ADULTERANTS IN TEA USING MID-INFRARED SPECTROSCOPY: A COMPARATIVE STUDY OF DEEP LEARNING AND MACHINE LEARNING',
  authors: [
...
const ArticleDetail = () => {
  return (
    <Layout>
    <div className="bg-white p-4 md:p-8 w-full font-serif">
      {/* Watermark in background */}
      <div className="relative border border-gray-300 p-6">
        <img 
        ...
        <div className="relative z-10">
          ...
          <div className="text-center mb-4">
            <h2 className="text-xl font-semibold text-blue-800">
              Volume {articleData.volume}, No. ({articleData.issue}), {articleData.year}, {articleData.month}
            </h2>
            <p className="text-gray-600">
              (Impact Factor, {articleData.impactFactor}; JCR {articleData.jcr})
            </p>
          </div>
          ...
          <div className="flex justify-end items-center space-x-4 mb-6 border-b-2 border-dotted border-gray-400 pb-4">
            <a href={articleData.pdfUrl} className="flex items-center text-red-700 hover:underline">
              <img src="https://www.thejaps.org.pk/images/pdf.png" alt="PDF Icon" className="h-6 mr-2"/>
              Download PDF
            </a>
            <a href={articleData.fullTextUrl} className="flex items-center text-orange-600 hover:underline">
               <img src="https://www.thejaps.org.pk/images/text.png" alt="Text Icon" className="h-6 mr-2" />
               Full Text Page
            </a>
          </div>
          ...
          <div className="mb-4">
            {articleData.authors.map((author, index) => (
              <span key={index} className="text-gray-800 font-semibold">
                {author.name}<sup>{author.superscript}</sup>{index < articleData.authors.length - 1 ? ', ' : ''}
              </span>
            ))}
          </div>
          ...
          <div className="space-y-1 text-sm text-gray-700 mb-4">
            {articleData.affiliations.map(aff => (
              <p key={aff.id}><sup>{aff.id}</sup>{aff.text}</p>
            ))}
          </div>
          
          {/* Corresponding Author & ORCID */}
          <div className="text-sm text-gray-700 space-y-1 mb-8">
             <p><sup>*</sup>Corresponding author’s Email: <a href={`mailto:${articleData.correspondingAuthorEmail}`} className="text-blue-600 hover:underline">{articleData.correspondingAuthorEmail}</a></p>
             <p>ORCID ID: {articleData.orcidId}</p>
          </div>

          {/* Abstract */}
          <div className="border-t border-b border-gray-300 py-6">
            <h3 className="text-center text-2xl font-bold text-purple-800 mb-4 tracking-widest">ABSTRACT</h3>
